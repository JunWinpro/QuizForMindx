...
const LEVEL_BG: Record<string, string> = {
  "Cơ bản": "rgba(69,183,209,.15)",
  "Trung cấp": "rgba(245,166,35,.15)",
  Advanced: "rgba(0,200,150,.15)",
};
    ...
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "40px 24px" }}>
      ...
      <div className="animate-fade-up" style={{ marginBottom: 32 }}>
        ...
        <p style={{ color: "var(--muted)", fontSize: 15 }}>
          Khám phá {decks.length}+ bộ từ vựng được tuyển chọn kỹ lưỡng
        </p>
        {user && (
          <Link
            to="/my-decks"
            style={{
              ...
              fontSize: 14,
              textDecoration: "none",
              boxShadow: "0 4px 16px rgba(13,27,42,.15)",
            }}
          >
            ✍️ Tạo deck của bạn
          </Link>
      ...
      >
        <div style={{ position: "relative", marginBottom: 16 }}>
          ...
          <input
            ...
            style={{ paddingLeft: 42, background: "var(--cream)" }}
          />
        </div>
        <div
          style={{
        ...
        >
          ...
          {levels.map((lv) => (
            <button
              key={lv}
              onClick={() => setLevel(lv)}
              className={`filter-pill${level === lv ? " active" : ""}`}
            >
              {lv === "all" ? "Tất cả" : lv}
            </button>
          ))}
